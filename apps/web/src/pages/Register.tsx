...
const Register: React.FC = () => {
    ...
    if (normalizedUsername && normalizedPassword) {
      ...
      if (res.success && res.userId) {
        ...
        if (inviteCode) {
          ...
          await fetchUserProfile(res.userId);
          const joinRes = await joinFamily(res.userId, inviteCode);
          if (joinRes.success) {
            alert('注册成功，家庭连接成功！');
          } else {
            alert(joinRes.message || '注册成功，但牵挂码加入失败，您可以稍后在家庭页面重新尝试');
          }
  ...
  return (
    <div className="min-h-screen bg-paper-100 flex flex-col items-center font-sans">
